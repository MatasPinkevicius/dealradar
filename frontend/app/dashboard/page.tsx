...
export default function Dashboard() {
  ...
  return (
    <main className="min-h-screen bg-gray-950 text-white">
      ...
      <div className="bg-gray-900 border-b border-gray-800 px-4 py-3 lg:px-6 lg:py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-3">
          <div className="flex items-center gap-2 min-w-0 lg:gap-4">
            <Link href="/" className="flex items-center gap-2 hover:opacity-80 transition-opacity shrink-0">
              <span className="text-lg lg:text-xl">📡</span>
              <span className="font-bold text-base lg:text-lg tracking-tight">DealRadar</span>
            </Link>
            <span className="text-gray-600 hidden lg:inline">|</span>
            <span className="text-gray-400 text-xs lg:text-sm truncate hidden sm:inline">{t.dash_subtitle}</span>
          </div>
          <div className="flex items-center gap-2 lg:gap-4 shrink-0">
            ...
            <div className="flex items-center gap-1 bg-gray-800 border border-gray-700 rounded-lg p-0.5 lg:p-1">
              <button onClick={() => setLang('lt')} className={`px-2 py-0.5 lg:px-3 lg:py-1 rounded-md text-xs lg:text-sm font-medium transition-colors ${lang === 'lt' ? 'bg-gray-700 text-white' : 'text-gray-400 hover:text-white'}`}>
                LT
              </button>
              <button onClick={() => setLang('en')} className={`px-2 py-0.5 lg:px-3 lg:py-1 rounded-md text-xs lg:text-sm font-medium transition-colors ${lang === 'en' ? 'bg-gray-700 text-white' : 'text-gray-400 hover:text-white'}`}>
                EN
              </button>
            </div>
          </div>
      ...
      <div className="bg-gray-900 border-b border-gray-800 px-4 py-3 lg:px-6">
        <div className="max-w-7xl mx-auto flex flex-col gap-3 lg:flex-row lg:flex-wrap lg:items-center">
          <input type="text" placeholder={t.dash_search} value={search} onChange={e => setSearch(e.target.value)} className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm w-full lg:w-56 focus:outline-none focus:ring-2 focus:ring-green-500 text-white placeholder-gray-500" />
          <select value={minScore} onChange={e => setMinScore(Number(e.target.value))} className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm w-full lg:w-auto focus:outline-none focus:ring-2 focus:ring-green-500 text-white">
          ...
          <input type="number" placeholder={lang === 'lt' ? 'Maks €' : 'Max €'} value={maxPrice} onChange={e => setMaxPrice(e.target.value)} className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm w-full lg:w-24 focus:outline-none focus:ring-2 focus:ring-green-500 text-white placeholder-gray-500" />
          <input type="number" placeholder={lang === 'lt' ? 'Min metai' : 'Min year'} value={minYear} onChange={e => setMinYear(e.target.value)} className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm w-full lg:w-28 focus:outline-none focus:ring-2 focus:ring-green-500 text-white placeholder-gray-500" />
          <select value={fuelType} onChange={e => setFuelType(e.target.value)} className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm w-full lg:w-auto focus:outline-none focus:ring-2 focus:ring-green-500 text-white">
          ...
          <select value={transmission} onChange={e => setTransmission(e.target.value)} className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm w-full lg:w-auto focus:outline-none focus:ring-2 focus:ring-green-500 text-white">
          ...
          <select value={bodyType} onChange={e => setBodyType(e.target.value)} className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm w-full lg:w-auto focus:outline-none focus:ring-2 focus:ring-green-500 text-white">
          ...
          <div className="flex items-center gap-2 w-full lg:w-auto">
            <span className="text-gray-400 text-sm shrink-0">{lang === 'lt' ? 'Rušiuoti:' : 'Sort:'}</span>
            <select value={sortBy} onChange={e => setSortBy(e.target.value)} className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm flex-1 lg:flex-none focus:outline-none focus:ring-2 focus:ring-green-500 text-white">
            ...
            <button onClick={() => setSortAsc(a => !a)} className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-300 hover:bg-gray-700 transition-colors shrink-0">
              {sortAsc ? '↑' : '↓'}
            </button>
          </div>
        </div>
              ...
              {filtered.map(listing => (
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-3">
                    {listing.year && (
                      <span className="bg-gray-800 text-gray-300 text-xs px-2 py-0.5 rounded-md">{listing.year}</span>
                    )}
                    {listing.mileage_km && (
                    ...
                    {listing.fuel_type && (
                      <span className="bg-gray-800 text-gray-300 text-xs px-2 py-0.5 rounded-md">{listing.fuel_type}</span>
                    )}
                  </div>
                  ...
                  <div className="text-sm text-gray-400 mb-2">
                    {listing.estimated_price ? (
                      <>
                        {t.dash_col_est}: €{listing.estimated_price.toLocaleString()}
                        {listing.price_vs_median != null && (
                          <span className={`ml-2 font-semibold ${listing.price_vs_median < 0 ? 'text-green-400' : 'text-red-400'}`}>
                  ...
                  <button
                    ...
                    className="w-full bg-green-500 hover:bg-green-400 text-gray-950 font-semibold py-2 rounded-lg text-sm transition-colors"
                  >
                    {t.dash_view}
                  </button>
                </article>
            ...
            <div className="hidden lg:block bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-800">
                  {filtered.map(listing => (
                    <tr key={listing.id} role="button" tabIndex={0} className="hover:bg-gray-800/50 transition-colors cursor-pointer group" onClick={() => openListing(listing.id)} onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); openListing(listing.id) } }}>
